...
### Changed
- **Migrated the app from .NET Framework 4.8 to .NET 8** (`net8.0-windows`). The project file is now SDK-style, so the build is `dotnet build src\Appcopier.sln` instead of `nuget restore` + `msbuild` from a Visual Studio Developer environment. **This changes the install requirement for end users: the app now needs the .NET Desktop Runtime 8 on the machine, where the 4.8 build ran on the Windows-bundled framework.** Build output also moves to `bin\<Configuration>\net8.0-windows\`.
...
- This `CHANGELOG.md`. (Developer tooling — `CLAUDE.md` and the `.claude/` automation — is deliberately kept out of the repository by `.gitignore` and is not part of any release.)
