---
name: release
description: Walk the Appcopier release flow - version bump in AssemblyInfo.cs (format is load-bearing for the in-app update checker), Release build, branch/commit, tag, and GitHub release.
...
Every installed copy of Appcopier checks for updates by downloading
...
2. **Bump version** in `src/Appcopier/Properties/AssemblyInfo.cs`: update both `AssemblyVersion` and `AssemblyFileVersion` to the same three-part value.
...
4. **Build and publish Release**, pasting the verbatim output of both:
   ```
   dotnet build src\Appcopier.sln -c Release
   dotnet test src\Appcopier.sln -c Release
   ```
   Do not proceed on a failed build or a failing test.

   **Then publish the release artifact.** Appcopier ships as a **self-contained single file**, so users
   download one `.exe` and run it with no .NET install — the same experience 0.30.0 had on .NET Framework.
   Use exactly these flags:
   ```
   dotnet publish src\Appcopier\Appcopier.csproj -c Release -r win-x64 --self-contained true ^
   ...
   Verify before shipping: `publish\` must contain **exactly one file**, `Appcopier.exe`, around 69 MB.
